...
/**
 * Sliding Window Log Rate Limiter 테스트
 * 성공/실패 케이스를 모두 검증합니다.
 */
@SpringBootTest
@Slf4j
@DisplayName("Sliding Window Log Rate Limiter 테스트")
class SlidingWindowLogRateLimiterTest {
    ...
    void setUp() {
        ...
        rateLimiter = new SlidingWindowLogRateLimiter(1, 5); // 1개 허용, 5초 윈도우
        request = new MockHttpServletRequest();
        log.info("Sliding Window Log Rate Limiter 초기화 완료 - limit: 1, windowSize: 5s");
    }

    @Test
    @DisplayName("성공 케이스 - 첫 번째 요청은 허용되어야 함")
    void testSuccessCase() {
        log.info("=== Sliding Window Log 성공 케이스 테스트 시작 ===");
        ...
        assertEquals("sliding-window-log", result.getAlgorithm(), "알고리즘 이름이 올바르지 않습니다");
        assertTrue(result.getResetTimeMillis() > 0, "리셋 시간이 설정되어야 합니다");
        
        log.info("Sliding Window Log 성공 테스트 완료 - remaining: {}, algorithm: {}", 
    ...
    void testFailureCase() {
        log.info("=== Sliding Window Log 실패 케이스 테스트 시작 ===");
        ...
        assertEquals("sliding-window-log", secondResult.getAlgorithm(), "알고리즘 이름이 올바르지 않습니다");
        assertTrue(secondResult.getRetryAfterSeconds() > 0, "재시도 시간이 설정되어야 합니다");
        
        log.info("Sliding Window Log 실패 테스트 완료 - 요청 거부됨, retryAfter: {}초", 
                secondResult.getRetryAfterSeconds());
    }

    @Test
    @DisplayName("슬라이딩 윈도우 테스트 - 오래된 요청이 제거되면 새 요청이 허용되어야 함")
    void testSlidingWindow() throws InterruptedException {
        log.info("=== Sliding Window Log 슬라이딩 윈도우 테스트 시작 ===");
        ...
        // 윈도우 시간이 지나도록 대기
        log.info("윈도우 만료를 위해 대기 중...");
        TimeUnit.SECONDS.sleep(6); // 윈도우 크기보다 조금 더 대기
        
        // 첫 번째 요청이 윈도우에서 제거되어 새 요청이 허용되어야 함
        RateLimitResult thirdResult = rateLimiter.allowRequest(testUserId, request);
        assertTrue(thirdResult.isAllowed(), "오래된 요청이 제거되어 새 요청이 허용되어야 합니다");
        
        log.info("Sliding Window Log 슬라이딩 윈도우 테스트 완료 - 윈도우 슬라이딩 후 요청 허용됨");
    }

    @Test
    @DisplayName("사용자별 격리 테스트 - 다른 사용자는 독립적으로 동작해야 함")
    void testUserIsolation() {
        log.info("=== Sliding Window Log 사용자별 격리 테스트 시작 ===");
        ...
        log.info("Sliding Window Log 사용자 격리 테스트 완료 - 사용자별 독립적 동작 확인");
    }

    @Test
    @DisplayName("리셋 기능 테스트 - 리셋 후 요청이 다시 허용되어야 함")
    void testResetFunction() {
        log.info("=== Sliding Window Log 리셋 기능 테스트 시작 ===");
        ...
        log.info("사용자 {}의 Sliding Window Log 리셋 완료", testUserId);
        ...
        log.info("Sliding Window Log 리셋 테스트 완료 - 리셋 후 정상 동작 확인");
    }

    @Test
    @DisplayName("다중 요청 처리 테스트 - 허용 한도 내에서 여러 요청 처리")
    void testMultipleRequestsWithHigherLimit() {
        log.info("=== Sliding Window Log 다중 요청 처리 테스트 시작 ===");
        
        // 더 큰 한도로 새 Rate Limiter 생성
        SlidingWindowLogRateLimiter multiRequestLimiter = new SlidingWindowLogRateLimiter(3, 10);
        ...
        log.info("Sliding Window Log 다중 요청 테스트 완료 - 한도 내 요청 허용, 초과 요청 거부");
    }

    @Test
    @DisplayName("로그 정리 기능 테스트 - 오래된 로그는 자동으로 정리되어야 함")
    void testLogCleanup() throws InterruptedException {
        log.info("=== Sliding Window Log 로그 정리 기능 테스트 시작 ===");
        
        // 더 큰 한도로 테스트 (로그 확인을 위해)
        SlidingWindowLogRateLimiter cleanupLimiter = new SlidingWindowLogRateLimiter(5, 3);
        
        // 여러 요청 실행
        for (int i = 1; i <= 3; i++) {
            RateLimitResult result = cleanupLimiter.allowRequest(testUserId, request);
            assertTrue(result.isAllowed(), i + "번째 요청은 허용되어야 합니다");
            TimeUnit.MILLISECONDS.sleep(500); // 요청 간 간격
        }
        
        // 윈도우 시간 경과 대기
        TimeUnit.SECONDS.sleep(4);
        
        // 새 요청 - 오래된 로그가 정리되어 허용되어야 함
        RateLimitResult newResult = cleanupLimiter.allowRequest(testUserId, request);
        assertTrue(newResult.isAllowed(), "로그 정리 후 새 요청이 허용되어야 합니다");
        
        log.info("Sliding Window Log 로그 정리 테스트 완료 - 자동 로그 정리 확인");
    }

    @Test
    @DisplayName("통계 기능 테스트 - 정확한 통계 정보를 반환해야 함")
    void testStatsFunction() {
        log.info("=== Sliding Window Log 통계 기능 테스트 시작 ===");
        ...
        assertEquals("sliding-window-log", stats.get("algorithm"), "알고리즘 이름이 올바르게 설정되어야 합니다");
        ...
        log.info("Sliding Window Log 통계 테스트 완료 - stats: {}", stats);
    }
}
